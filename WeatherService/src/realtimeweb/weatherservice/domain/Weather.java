...
public class Weather {
    ...
    public Weather(Map<String, Object> raw) {
        ...
        try {
        	this.windSpeed = isInteger(raw.get("Winds").toString(), 10) ? 
        			Integer.parseInt(raw.get("Winds").toString()) : 0;
                    this.windchill = 0; //Integer.parseInt(raw.get("WindChill").toString());
            this.dewpoint = isInteger(raw.get("Dewp").toString(), 10) ?
            		Integer.parseInt(raw.get("Dewp").toString()) : 0;
            this.imageUrl = raw.get("Weatherimage").toString();
            this.windDirection = isInteger(raw.get("Windd").toString(), 10) ?
            		Integer.parseInt(raw.get("Windd").toString()) : 0;
            this.visibility = isInteger(raw.get("Visibility").toString(), 10) ?
            		Double.parseDouble(raw.get("Visibility").toString()) : 0;
            this.humidity = isInteger(raw.get("Relh").toString(), 10) ?
            		Integer.parseInt(raw.get("Relh").toString()) : 0;
            this.pressure = isInteger(raw.get("SLP").toString(), 10) ?
            		Double.parseDouble(raw.get("SLP").toString()) : 0;
            this.temp = isInteger(raw.get("Temp").toString(), 10) ?
            		Integer.parseInt(raw.get("Temp").toString()) : 0;
...
//            this.description = raw.get("Weather").toString();
    		...
    		System.err.println("Could not convert the response to a Weather; a field had the wrong type.");
    		e.printStackTrace();
        }
